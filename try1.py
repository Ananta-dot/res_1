...
def height_correct(rects, colors):
    ...
    by_color = {}
    for idx, ((_, _), (y1, _)) in enumerate(rects):
...
def calc_score(state, n):
    comp_bits_H      = state[:m]
    comp_bits_V      = state[m:2*m]
    color_bits_total = state[2*m:]
    colors           = bits_to_colors(color_bits_total)

    base_arr = build_base_array(n)
    arrH     = apply_comps(base_arr, comp_bits_H, comps)
    arrV     = apply_comps(base_arr, comp_bits_V, comps)
    ...
    rectangles_corrected = height_correct(rectangles, colors)
    lp_val, ilp_val, ratio = solve_disjoint_rectangles(rectangles_corrected)
    return ratio, arrH, arrV, colors
...
def play_game(n_sess, actions, state_next, states, prob, step, total_score):
    for i in range(n_sess):
        ...
        if not terminal:
            ...
            ratio, arrH, arrV, colors = calc_score(state_next[i][:DECISIONS], n)
            total_score[i] = ratio
        if not terminal:
...
for i in range(min(3, len(super_rewards))):
